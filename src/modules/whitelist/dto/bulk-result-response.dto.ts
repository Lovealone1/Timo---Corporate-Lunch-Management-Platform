...
class BulkErrorItem {
    ...
    @ApiProperty({ example: '1234567890', description: 'CC value of the failed row' })
    cc!: string;
...
export class BulkResultResponseDto {
    @ApiProperty({ example: 10, description: 'Number of entries successfully created' })
    created!: number;

    @ApiProperty({ example: 2, description: 'Number of entries skipped (duplicates)' })
    skipped!: number;
