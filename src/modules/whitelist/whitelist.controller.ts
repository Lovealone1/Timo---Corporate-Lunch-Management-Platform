...
export class WhitelistController {
    ...
    @ApiQuery({ name: 'q', required: false, description: 'Search by name or cc (contains, case-insensitive)' })
    ...
    @ApiBadRequestResponse({ description: 'Validation error or no fields provided' })
    @ApiConflictResponse({ description: 'CC already exists' })
    @ApiNotFoundResponse({ description: 'Whitelist entry not found' })
    update(
        @Param('id') id: string,
    ...
    @ApiNotFoundResponse({ description: 'Whitelist entry not found' })
    @ApiConflictResponse({ description: 'Cannot delete: entry is referenced by reservations' })
    ...
    @ApiOperation({ summary: 'Bulk create whitelist entries from XLSX or CSV file' })
    ...
    @ApiBadRequestResponse({ description: 'No file uploaded or invalid file format' })
    async bulkCreate(
        ...
        if (!allowed.includes(file.mimetype)) {
            throw new BadRequestException('Invalid file format. Only XLSX and CSV files are allowed.');
        }

        return this.whitelist.bulkCreate(file.buffer, file.mimetype);
    }
}
