...
export class WhitelistService {
    ...
    async create(dto: CreateWhitelistDto) {
        ...
        try {
            ...
            if (e?.code === 'P2002') throw new ConflictException('A whitelist entry with this cc already exists');
            throw e;
        }
    }

    async findAll(params: { q?: string; enabled?: boolean; skip?: number; take?: number }) {
    ...
    async update(id: string, dto: UpdateWhitelistDto) {
        ...
        if (Object.keys(data).length === 0) {
            throw new BadRequestException('At least one field (cc or name) must be provided');
        }

        const exists = await this.prisma.whitelistEntry.findUnique({ where: { id }, select: { id: true } });
    ...
    async deactivate(id: string) {
        const entry = await this.prisma.whitelistEntry.findUnique({ where: { id }, select: { id: true, enabled: true } });
    ...
    async delete(id: string) {
        ...
        try {
            ...
            if (e?.code === 'P2003') {
                throw new ConflictException('Cannot delete: whitelist entry is referenced by reservations. Deactivate it instead.');
            }
            throw e;
        }
    }

    async bulkCreate(buffer: Buffer, mimetype: string) {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            const name = String(row['name'] ?? row['Name'] ?? row['NAME'] ?? row['nombre'] ?? row['Nombre'] ?? '').trim();

            if (!cc || cc.length < 2) {
                errors.push({ row: i + 2, cc: cc || '(empty)', reason: 'Missing or invalid cc' });
                continue;
            }
