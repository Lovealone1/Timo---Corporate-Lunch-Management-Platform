...
describe('MenusService', () => {
  ...
  describe('create', () => {
    ...
    it('should create protein and side options when provided', async () => {
      ...
      const call = prisma.menu.create.mock.calls[0][0] as {
        data: { proteinOptions: unknown; sideOptions: unknown };
      };
      expect(call.data.proteinOptions).toEqual({
        create: [{ proteinTypeId: 'prot-a' }, { proteinTypeId: 'prot-b' }],
      });
  ...
  describe('findByDate', () => {
    ...
    it('should throw NotFoundException when no menu for date', async () => {
      ...
      await expect(service.findByDate('2026-12-25')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('clone', () => {
    ...
    it('should throw NotFoundException when source does not exist', async () => {
      ...
      await expect(service.clone('nope', '2026-03-11')).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe('update', () => {
    it('should update scalar fields within a transaction', async () => {
      ...
      expect(mockTx.menu.update).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { id: 'menu-1' },
          data: expect.objectContaining({ soupId: 'soup-2' }) as Record<
            string,
            unknown
          >,
        }),
    ...
    it('should replace protein options in a transaction', async () => {
      ...
      await service.update('menu-1', {
        proteinOptionIds: ['prot-x', 'prot-y'],
      });
  ...
  describe('delete', () => {
    it('should delete and return { deleted: true, id }', async () => {
      ...
      expect(await service.delete('menu-1')).toEqual({
        deleted: true,
        id: 'menu-1',
      });
    });
