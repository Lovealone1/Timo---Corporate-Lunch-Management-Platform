...
import { isDateInPastColombia, colombiaTimestamps, colombiaUpdatedAt } from '../../common/date.util';
...
export class MenusService {
    ...
    async create(dto: CreateMenuDto) {
        ...
        const defaultProteinTypeId = dto.defaultProteinTypeId ?? DEFAULT_PROTEIN_TYPE_ID;
            ...
            return await this.prisma.menu.create({
                data: {
                    ...
                    proteinOptions: dto.proteinOptionIds?.length
                        ? { create: dto.proteinOptionIds.map((id) => ({ proteinTypeId: id })) }
                        : undefined,
    ...
    async clone(id: string, targetDateStr: string) {
        ...
        if (isDateInPastColombia(targetDateStr)) {
            throw new BadRequestException('Cannot clone a menu to a past date (Colombia timezone)');
        }
            ...
            return await this.prisma.menu.create({
                data: {
                    ...
                    proteinOptions: source.proteinOptions.length
                        ? { create: source.proteinOptions.map((o) => ({ proteinTypeId: o.proteinTypeId })) }
                        : undefined,
                    sideOptions: source.sideOptions.length
                        ? { create: source.sideOptions.map((o) => ({ sideDishId: o.sideDishId })) }
                        : undefined,
                    ...colombiaTimestamps(),
                },
                include: INCLUDE_RELATIONS,
    ...
    async update(id: string, dto: UpdateMenuDto) {
        ...
        try {
            return await this.prisma.$transaction(async (tx) => {
                ...
                if (dto.defaultProteinTypeId !== undefined) data.defaultProteinTypeId = dto.defaultProteinTypeId;

                if (Object.keys(data).length > 0) {
                    await tx.menu.update({ where: { id }, data: { ...data, ...colombiaUpdatedAt() } });
                }
                ...
                if (dto.proteinOptionIds !== undefined) {
                    ...
                    if (dto.proteinOptionIds.length > 0) {
                        await tx.menuProteinOption.createMany({
                            data: dto.proteinOptionIds.map((proteinTypeId) => ({ menuId: id, proteinTypeId })),
                        });
                    }
                }
                ...
                if (dto.sideOptionIds !== undefined) {
                    ...
                    if (dto.sideOptionIds.length > 0) {
                        await tx.menuSideOption.createMany({
                            data: dto.sideOptionIds.map((sideDishId) => ({ menuId: id, sideDishId })),
                        });
                    }
                }
