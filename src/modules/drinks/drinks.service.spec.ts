...
const fakeDrink = (overrides: Partial<Record<string, any>> = {}) => ({
    id: 'drink-1',
    name: 'Limonada',
    isActive: true,
...
describe('DrinksService', () => {
    ...
    describe('create', () => {
        it('should create a drink and return it', async () => {
            ...
            expect(prisma.drink.create).toHaveBeenCalledWith(
                expect.objectContaining({
                    data: expect.objectContaining({ name: 'Limonada', isActive: true }),
                }),
            );
        });
        ...
        it('should trim the name', async () => {
            ...
            expect(prisma.drink.create).toHaveBeenCalledWith(
                expect.objectContaining({
                    data: expect.objectContaining({ name: 'Limonada' }),
                }),
            );
        });
        ...
        it('should throw ConflictException on duplicate name (P2002)', async () => {
            ...
            await expect(service.create({ name: 'Limonada' })).rejects.toThrow(ConflictException);
        });

        it('should rethrow non-P2002 errors', async () => {
            prisma.drink.create.mockRejectedValue(new Error('DB down'));

            await expect(service.create({ name: 'Limonada' })).rejects.toThrow('DB down');
        });
    });
    ...
    describe('findAll', () => {
        it('should return list with default pagination', async () => {
            const items = [fakeDrink(), fakeDrink({ id: 'drink-2', name: 'Jugo de mora' })];
        ...
        it('should filter by search query', async () => {
            ...
            const call = prisma.drink.findMany.mock.calls[0][0];
    ...
    describe('delete', () => {
        it('should delete and return { deleted: true, id }', async () => {
            ...
            expect(await service.delete('drink-1')).toEqual({ deleted: true, id: 'drink-1' });
        });
        ...
        it('should throw ConflictException when referenced (P2003)', async () => {
            ...
            await expect(service.delete('drink-1')).rejects.toThrow(ConflictException);
        });
