...
const fakeSideDish = (overrides: Partial<Record<string, any>> = {}) => ({
    id: 'sd-1',
    name: 'Arroz blanco',
    isActive: true,
...
describe('SideDishesService', () => {
    ...
    describe('create', () => {
        it('should create a side dish and return it', async () => {
            ...
            expect(prisma.sideDish.create).toHaveBeenCalledWith(
                expect.objectContaining({
                    data: expect.objectContaining({ name: 'Arroz blanco', isActive: true }),
                }),
            );
        });
        ...
        it('should trim the name', async () => {
            ...
            expect(prisma.sideDish.create).toHaveBeenCalledWith(
                expect.objectContaining({
                    data: expect.objectContaining({ name: 'Arroz blanco' }),
                }),
            );
        });

        it('should allow setting isActive to false', async () => {
            prisma.sideDish.create.mockResolvedValue(fakeSideDish({ isActive: false }));
        ...
        it('should throw ConflictException on duplicate name (P2002)', async () => {
            ...
            await expect(service.create({ name: 'Arroz blanco' })).rejects.toThrow(ConflictException);
        });

        it('should rethrow non-P2002 errors', async () => {
            prisma.sideDish.create.mockRejectedValue(new Error('DB down'));

            await expect(service.create({ name: 'Arroz blanco' })).rejects.toThrow('DB down');
        });
    });
    ...
    describe('findAll', () => {
        it('should return list with default pagination', async () => {
            const items = [fakeSideDish(), fakeSideDish({ id: 'sd-2', name: 'Ensalada' })];
        ...
        it('should filter by search query', async () => {
            ...
            const call = prisma.sideDish.findMany.mock.calls[0][0];
    ...
    describe('deactivate', () => {
        it('should set isActive to false', async () => {
            ...
            const result = await service.deactivate('sd-1');
    ...
    describe('delete', () => {
        it('should delete and return { deleted: true, id }', async () => {
            ...
            expect(await service.delete('sd-1')).toEqual({ deleted: true, id: 'sd-1' });
        });
