...
export class UsersService {
  ...
  async findAll(
    ...
    this.logger.log(
      `LIST users — page=${page} limit=${limit} q=${q ?? '(none)'}`,
    );
  ...
  async updateRole(id: string, role: string): Promise<UserResponseDto> {
    ...
    return this.prisma.profile.update({
      where: { id },
      data: { role: role as Role, updatedAt: now },
    });
  }
  ...
  async create(dto: CreateUserDto): Promise<UserResponseDto> {
    ...
    try {
      ...
      this.logger.error('Supabase user creation failed', err.message);
      if (err.message?.includes('already been registered')) {
        throw new ConflictException(
          `Email ${dto.email} already registered in Auth`,
        );
      }
      throw error;
    }
    ...
    try {
      ...
      const pe = error as PrismaError;
      // Handle unique constraint violations gracefully
      if (pe.code === 'P2002') {
        throw new ConflictException(
          `Profile already exists (duplicate on: ${String(pe.meta?.target)})`,
        );
      }
