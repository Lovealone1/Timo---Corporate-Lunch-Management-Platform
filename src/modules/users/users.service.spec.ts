...
/* ───────── Helpers ───────── */
const fakeProfile = (overrides: Partial<Record<string, any>> = {}) => ({
    id: 'sup-uuid-1',
...
describe('UsersService', () => {
    ...
    beforeEach(async () => {
        ...
        supabase = module.get(SupabaseAdminService) as unknown as MockSupabase;
    });
    ...
    describe('create', () => {
        ...
        it('should create user in Supabase and upsert profile', async () => {
            ...
            const profile = fakeProfile({ id: 'sup-new-id', email: 'new@example.com' });
            prisma.profile.upsert.mockResolvedValue(profile);

            const result = await service.create(dto);

            expect(result).toEqual(profile);
            expect(supabase.createAuthUser).toHaveBeenCalledWith('new@example.com', 'Str0ngP@ss');
        ...
        it('should use provided role and enabled values', async () => {
            prisma.profile.findUnique.mockResolvedValue(null);
            supabase.createAuthUser.mockResolvedValue(supabaseUser);
            prisma.profile.upsert.mockResolvedValue(fakeProfile({ role: 'ADMIN', enabled: false }));

            await service.create({ ...dto, role: 'ADMIN' as any, enabled: false });

            expect(prisma.profile.upsert).toHaveBeenCalledWith(
                expect.objectContaining({
                    create: expect.objectContaining({ role: 'ADMIN', enabled: false }),
                    update: expect.objectContaining({ role: 'ADMIN', enabled: false }),
                }),
            );
        });
        ...
        it('should rethrow unknown Supabase errors', async () => {
            ...
            await expect(service.create(dto)).rejects.toThrow('Supabase network timeout');
        });
