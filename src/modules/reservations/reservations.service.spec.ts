...
/* ───────── Helpers ───────── */
const fakeUser = (overrides: Partial<Record<string, any>> = {}) => ({
    id: 'wl-1', cc: '123456', name: 'John Doe', enabled: true, ...overrides,
});
...
const fakeReservation = (overrides: Partial<Record<string, any>> = {}) => ({
    id: 'res-1',
    menuId: 'menu-1',
    cc: '123456',
    name: 'John Doe',
    ...
    menu: { id: 'menu-1', date: new Date('2026-03-10T00:00:00Z'), dayOfWeek: 'MAR' },
    sideDishes: [],
    createdAt: new Date('2026-01-01T05:00:00Z'),
    updatedAt: new Date('2026-01-01T05:00:00Z'),
    ...overrides,
...
describe('ReservationsService', () => {
    ...
    describe('create', () => {
        ...
        it('should auto-assign default protein for same-day reservations', async () => {
            (dateUtil.isDateTomorrowOrLaterColombia as jest.Mock).mockReturnValue(false);
            prisma.whitelistEntry.findUnique.mockResolvedValue(fakeUser());
            prisma.menu.findUnique.mockResolvedValue(fakeMenu());
            prisma.reservation.create.mockResolvedValue(
                fakeReservation({ proteinTypeId: 'prot-default', status: 'AUTO_ASIGNADA' }),
            );
        ...
        it('should throw ForbiddenException when user is disabled', async () => {
            prisma.whitelistEntry.findUnique.mockResolvedValue(fakeUser({ enabled: false }));
        ...
        it('should throw BadRequestException when no default protein on same-day', async () => {
            (dateUtil.isDateTomorrowOrLaterColombia as jest.Mock).mockReturnValue(false);
            prisma.whitelistEntry.findUnique.mockResolvedValue(fakeUser());
            prisma.menu.findUnique.mockResolvedValue(fakeMenu({ defaultProteinTypeId: null }));
    ...
    describe('update', () => {
        ...
        it('should update protein in a transaction', async () => {
            ...
            expect(mockTx.reservation.update).toHaveBeenCalledWith(
                expect.objectContaining({
                    data: expect.objectContaining({ proteinTypeId: 'prot-b' }),
                }),
            );
        });
        ...
        it('should replace side dishes when provided', async () => {
            prisma.reservation.findUnique.mockResolvedValue(reservationWithMenu);
            mockTx.sideDish.findMany.mockResolvedValue([{ id: 'sd-1', name: 'Arroz' }]);
            ...
            expect(mockTx.reservationSideDish.createMany).toHaveBeenCalledWith({
                data: [{ reservationId: 'res-1', sideDishId: 'sd-1', nameSnapshot: 'Arroz' }],
            });
        });
        ...
        it('should throw NotFoundException when not found', async () => {
            ...
            await expect(service.update('nope', dto)).rejects.toThrow(NotFoundException);
        });

        it('should throw ForbiddenException on cc mismatch', async () => {
            prisma.reservation.findUnique.mockResolvedValue({ ...reservationWithMenu, cc: '999999' });

            await expect(service.update('res-1', dto)).rejects.toThrow(ForbiddenException);
        });
        ...
        it('should throw BadRequestException for same-day/past modifications', async () => {
            ...
            await expect(service.update('res-1', dto)).rejects.toThrow(BadRequestException);
        });

        it('should throw BadRequestException when reservation is cancelled', async () => {
            prisma.reservation.findUnique.mockResolvedValue({
                ...reservationWithMenu, status: 'CANCELADA',
            });
    ...
    describe('cancel', () => {
        const reservation = {
            id: 'res-1', cc: '123456', status: 'RESERVADA',
        ...
        it('should throw NotFoundException when not found', async () => {
            ...
            await expect(service.cancel('nope', '123456')).rejects.toThrow(NotFoundException);
        });

        it('should throw ForbiddenException on cc mismatch', async () => {
            prisma.reservation.findUnique.mockResolvedValue(reservation);

            await expect(service.cancel('res-1', '999999')).rejects.toThrow(ForbiddenException);
        });
        ...
        it('should throw BadRequestException for same-day/past cancellations', async () => {
            ...
            await expect(service.cancel('res-1', '123456')).rejects.toThrow(BadRequestException);
        });

        it('should throw BadRequestException when already cancelled', async () => {
            prisma.reservation.findUnique.mockResolvedValue({ ...reservation, status: 'CANCELADA' });
    ...
    describe('delete', () => {
        it('should delete and return { deleted: true, id }', async () => {
            ...
            expect(await service.delete('res-1')).toEqual({ deleted: true, id: 'res-1' });
        });
    ...
    describe('findAll', () => {
        ...
        it('should filter by date when provided', async () => {
            ...
            expect(call.where).toEqual({ menu: { date: new Date('2026-03-10T00:00:00Z') } });
        });
    ...
    describe('findSummaryByDate', () => {
        it('should return summary with protein counts grouped', async () => {
            ...
            prisma.reservation.findMany.mockResolvedValue([
                { proteinTypeId: 'p1', proteinType: { name: 'Pollo' }, status: 'RESERVADA' },
                { proteinTypeId: 'p1', proteinType: { name: 'Pollo' }, status: 'RESERVADA' },
                { proteinTypeId: 'p2', proteinType: { name: 'Res' }, status: 'AUTO_ASIGNADA' },
            ]);
        ...
        it('should exclude CANCELADA from protein counts', async () => {
            ...
            prisma.reservation.findMany.mockResolvedValue([
                { proteinTypeId: 'p1', proteinType: { name: 'Pollo' }, status: 'RESERVADA' },
                { proteinTypeId: 'p1', proteinType: { name: 'Pollo' }, status: 'CANCELADA' },
            ]);
        ...
        it('should return SERVIDA when any reservation is served', async () => {
            prisma.menu.findUnique.mockResolvedValue({ id: 'menu-1' });
            prisma.reservation.findMany.mockResolvedValue([
                { proteinTypeId: 'p1', proteinType: { name: 'Pollo' }, status: 'SERVIDA' },
                { proteinTypeId: 'p1', proteinType: { name: 'Pollo' }, status: 'RESERVADA' },
            ]);
        ...
        it('should throw NotFoundException when no menu for date', async () => {
            ...
            await expect(service.findSummaryByDate('2026-12-25')).rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe('bulkMarkServed', () => {
        it('should mark non-cancelled reservations as SERVIDA', async () => {
            ...
            expect(result).toEqual({ date: '2026-03-10', status: 'SERVIDA', updated: 5 });
            expect(prisma.reservation.updateMany).toHaveBeenCalledWith(
                expect.objectContaining({
                    where: { menuId: 'menu-1', status: { not: 'CANCELADA' } },
                    data: expect.objectContaining({ status: 'SERVIDA' }),
                }),
            );
        });
        ...
        it('should throw NotFoundException when no menu for date', async () => {
            ...
            await expect(service.bulkMarkServed('2026-12-25')).rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe('bulkMarkCancelled', () => {
        it('should mark eligible reservations as CANCELADA', async () => {
            ...
            expect(result).toEqual({ date: '2026-03-10', status: 'CANCELADA', updated: 3 });
            expect(prisma.reservation.updateMany).toHaveBeenCalledWith(
                expect.objectContaining({
                    where: { menuId: 'menu-1', status: { notIn: ['CANCELADA', 'SERVIDA'] } },
                }),
            );
        });
        ...
        it('should throw NotFoundException when no menu for date', async () => {
            ...
            await expect(service.bulkMarkCancelled('2026-12-25')).rejects.toThrow(NotFoundException);
        });
    });
});
